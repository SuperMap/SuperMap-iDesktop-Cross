...
public class LayerGridParamColorTableDialog extends SmDialog {
	...
	private LayerSettingGrid currentLayerSettingGrid;
	//by yuanR 实现面板和model联动的接口
	private ColorsDictionarySet colorsDictionarySet = null;

	public LayerGridParamColorTableDialog(LayerSettingGrid layerSettingGrid, double[] inputOriginKeys, ColorsDictionarySet colorsDictionarySet) {
		this(layerSettingGrid, inputOriginKeys);
		this.colorsDictionarySet = colorsDictionarySet;
	}
	...
	private void initComponents() {
		...
		buttonInputColorTable.setToolTipText(ControlsProperties.getString("String_InputColorTable"));
		...
		buttonExportColorTable.setToolTipText(ControlsProperties.getString("String_ExprotColorTable"));
		...
		buttonDefaultColotTable.setToolTipText(ControlsProperties.getString("String_DefaultColorTable"));
	...
	private void tableChange() {
		...
		if (colorsDictionarySet != null) {
			colorsDictionarySet.set(keys, colors);
		} else {
	...
	public void actionCanceled() {
		//还原图层
		if (colorsDictionarySet != null) {
			colorsDictionarySet.set(keysOrigin, colorsOrigin);
			colorsDictionarySet.refresh();
		} else {
	...
	private void inputColorTable() {
		...
		if (state == JFileChooser.APPROVE_OPTION) {
			...
			if (colorDictionary != null) {
				...
				Application.getActiveApplication().getOutput().output(ControlsProperties.getString("String_DialogColorTableInputSucess") + filePath);
			} else {
				Application.getActiveApplication().getOutput().output(ControlsProperties.getString("String_DialogColorTableInputFailed"));
			}
		}
	}
