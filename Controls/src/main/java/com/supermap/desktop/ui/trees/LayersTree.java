...
public class LayersTree extends JTree {
    ...
    private transient LayersTreeExpansionListener layersTreeExpansionListener = null;
...
//        }
    ...
    private void unRegisterListeners() {
        if (currentMap.getWorkspace() != null) {
            ...
            this.removeTreeExpansionListener(layersTreeExpansionListener);
        }
    }
    ...
    private void registerListeners() {
        if (currentMap.getWorkspace() != null) {
            ...
            this.addTreeExpansionListener(getLayersTreeExpansionListener());
        }
    }
...
//			dropTargetNodeIndex = ((DefaultMutableTreeNode) getModel().getRoot()).getIndex(dropTargetNode);
    ...
    private void setLayersTreeExpansionChanged(TreeExpansionEvent event, boolean isExpand) {
        ...
        if (node != null) {
            ...
            IFormMap iFormMap = (IFormMap) Application.getActiveApplication().getActiveForm();
            ArrayList<LayerGroup> expandLayerGroupsList = new ArrayList<>();
            if (!this.expandLayerGroups.containsKey(iFormMap)) {
                this.expandLayerGroups.put(iFormMap, expandLayerGroupsList);
            } else {
                expandLayerGroupsList = this.expandLayerGroups.get(iFormMap);
            }
            if (nodeData.getData() instanceof LayerGroup) {
                if (isExpand) {
                    if (!expandLayerGroupsList.contains((LayerGroup) nodeData.getData())) {
                        expandLayerGroupsList.add((LayerGroup) nodeData.getData());
                    }
                } else {
                    if (expandLayerGroupsList.contains((LayerGroup) nodeData.getData())) {
                        expandLayerGroupsList.remove((LayerGroup) nodeData.getData());
                    }
                }
            }
        }
    ...
    public LayerGroup[] getExpandLayerGroup(IFormMap iFormMap) {
        LayerGroup[] layerGroups = new LayerGroup[this.expandLayerGroups.get(iFormMap).size()];
        this.expandLayerGroups.get(iFormMap).toArray(layerGroups);
        return layerGroups;
    }
