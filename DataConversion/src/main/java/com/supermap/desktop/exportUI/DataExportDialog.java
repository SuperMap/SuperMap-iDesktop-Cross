...
*/
	...
	public void addTableInfo(Dataset[] datasets) {
		...
		for (Dataset dataset : datasets) {
			...
			if (exportSetting.getSourceData() instanceof DatasetVector) {
				isGpx = GPXAnalytic.isGPXType((DatasetVector) exportSetting.getSourceData());
			}
	...
	public void initComboBoxColumns() {
		...
		for (int i = 0; i < this.panelExports.size(); i++) {
			...
			if (exportSetting.getSourceData() instanceof DatasetVector && GPXAnalytic.isGPXType((DatasetVector) exportSetting.getSourceData())) {
				this.steppedComboBox.addItem(DataConversionProperties.getString("String_FileTypeGPS"));
			}
			...
			this.itemListener = new ItemListener() {
				...
				public void itemStateChanged(ItemEvent e) {
					if (e.getStateChange() == ItemEvent.SELECTED) {
						...
						Object fileType = FiletypeUtilities.getFileType(fileTypeAlias);
