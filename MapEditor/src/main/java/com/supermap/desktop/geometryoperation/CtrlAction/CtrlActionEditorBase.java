...
public class CtrlActionEditorBase extends CtrlAction {
	...
	public boolean enable() {
		...
		if (null != PluginEnvironment.getGeometryEditManager().instance()) {
            flag = this.getEditor().enable(PluginEnvironment.getGeometryEditManager().instance());
        }
        return flag;
	...
	public boolean check() {
		boolean flag = false;
		if (null != PluginEnvironment.getGeometryEditManager().instance()) {
			flag = this.getEditor().check(PluginEnvironment.getGeometryEditManager().instance());
		}
		return flag;
	}
}
