...
import com.supermap.desktop.utilities.SnapSettingUtilities;
	...
	private ActionListener recoverListener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (panelSnapMode.isVisible()) {
				srcSnapSetting = snapSetting;
				initTable();
				srcSnapSetting.setTolerance(Double.valueOf(textFieldSnapTolarence.getText()).intValue());
				srcSnapSetting.setFixedAngle(Double.valueOf(textFieldFixedAngle.getText()));
				srcSnapSetting.setMinSnappedLength(Double.valueOf(textFieldMinSnappedLength.getText()).intValue());
				srcSnapSetting.setFixedLength(Double.valueOf(textFieldFixedLength.getText()));
				srcSnapSetting.setSnappedLineBroken(checkBoxSnappedLineBroken.isSelected());
				isRecoverSnapMode = true;
			} else {
				textFieldSnapTolarence.setText(format.format(snapSetting.getTolerance()));
				textFieldFixedAngle.setText(format.format(snapSetting.getFixedAngle()));
//            textFieldMaxSnappedCount.setText(format.format(srcSnapSetting.getMaxSnappedCount()));
				textFieldMinSnappedLength.setText(format.format(snapSetting.getMinSnappedLength()));
				textFieldFixedLength.setText(format.format(snapSetting.getFixedLength()));
	...
	private ActionListener okListener = new ActionListener() {
		@Override
		public void actionPerformed(ActionEvent e) {
			SnapSettingUtilities.setSnapSetting(mapControlSnapSetting);
			IFormManager formManager = Application.getActiveApplication().getMainFrame().getFormManager();
			for (int i = 0; i < formManager.getCount(); i++) {
				if (formManager.get(i).getWindowType() == WindowType.MAP) {
					((IFormMap) formManager.get(i)).getMapControl().setSnapSetting(mapControlSnapSetting);
				}
			}
			removeEvents();
			isRecoverSnapMode = false;
	...
	public SnapSettingDialog() {
		initSnapSetting();
	}

	private void initSnapSetting() {
		srcSnapSetting = new SnapSetting(SnapSettingUtilities.getSnapSetting());
...
//        this.labelMaxSnappedCount.setText(DataEditorProperties.getString("String_SnapSettingMaxSnappedCount"));
	...
	private void initLayout() {
		...
		this.tabbedPane.addTab(CoreProperties.getString("String_parameter"), panelSnapParams);
