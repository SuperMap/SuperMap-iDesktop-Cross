...
public class MapBasePropertyControl extends AbstractPropertyControl {
	...
	private JCheckBox checkBoxGraphicsAlpha;
	...
	private boolean isGraphicsAlphaEnabled = false;
	...
	private void absractItemStateChanged(ItemEvent e) {
		...
		} else if (e.getSource() == checkBoxGraphicsAlpha) {
			checkBoxIsGraphicsAlphaEnabled();
	...
	protected void initializeComponents() {
		...
		this.checkBoxGraphicsAlpha = new JCheckBox("GraphicsAlpha");
		...
		gl_displayedOptions.setHorizontalGroup(gl_displayedOptions.createSequentialGroup()
				...
				.addGroup(gl_displayedOptions.createParallelGroup(Alignment.LEADING)
						...
						.addComponent(this.checkBoxGraphicsAlpha)));
	...
	public void apply() {
		if (getMap() != null) {
			...
			getMap().setGraphicsAlphaEnable(this.isGraphicsAlphaEnabled);
	...
	protected void initializeResources() {
		...
		this.checkBoxGraphicsAlpha.setText(MapViewProperties.getString("String_CheckBox_IsGraphicsAlphaEnabled"));
	...
	protected void initializePropertyValues(Map map) {
		...
		this.isGraphicsAlphaEnabled = map.isGraphicsAlphaEnable();
	...
	protected void registerEvents() {
		...
		this.checkBoxGraphicsAlpha.addItemListener(this.itemListener);
	...
	protected void unregisterEvents() {
		...
		this.checkBoxGraphicsAlpha.removeItemListener(this.itemListener);
	...
	protected void fillComponents() {
		...
		this.checkBoxGraphicsAlpha.setSelected(this.isGraphicsAlphaEnabled);
	...
	protected boolean verifyChange() {
		return Double.compare(getMap().getAngle(), this.angle) != 0 || getMap().getColorMode() != this.colorMode
				...
				|| getMap().isGraphicsAlphaEnable() != this.isGraphicsAlphaEnabled
	...
	private void textFieldMaxVisibleVertexValueChange(PropertyChangeEvent e) {
		try {
			if ("value".equalsIgnoreCase(e.getPropertyName())) {
				if (e.getNewValue().toString().length() < 10) {
	...
	private void checkBoxIsGraphicsAlphaEnabled() {
		try {
			this.isGraphicsAlphaEnabled = this.checkBoxGraphicsAlpha.isSelected();
			verify();
		} catch (Exception e2) {
			Application.getActiveApplication().getOutput().output(e2);
		}
