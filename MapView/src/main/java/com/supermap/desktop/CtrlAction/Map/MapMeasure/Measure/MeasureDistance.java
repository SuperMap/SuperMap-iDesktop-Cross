...
public class MeasureDistance extends Measure {
    ...
    private static final String measureLineTag = TRACKING_OBJECT_NAME + "MeasureLineTag";
    /**
            ...
            public void tracked(TrackedEvent e) {
                try {
                    if (e.getGeometry() != null) {
                        ...
                        mapControl.getMap().getTrackingLayer().add(e.getGeometry(), TRACKING_OBJECT_NAME + measureLineTag);
    ...
    private void setDistantTextBox(TrackingEvent trackingEvent, String unitName) {
        try {
            ...
            double totalLength = computeDistance(trackingEvent, LengthUnit.getValueOf(unitName).getUnit(), true);
            double curLength = computeDistance(trackingEvent, LengthUnit.getValueOf(unitName).getUnit(), false);
    ...
    private void drawDistanceText(Point2Ds points, String unitString, int param, boolean isDrawing) {
        ...
        for (int i = 1; i < points.getCount() - param; i++) {
            ...
            if (!addedTags.contains(tag)) {
                ...
                distance = computeDistance(pntA, pntB, curDistanceUnit);
    ...
    protected double computeDistance(TrackingEvent event, Unit unit, boolean isTotal) {
        return LengthUnit.ConvertDistance(mapControl.getMap().getPrjCoordSys(), unit, isTotal ? event.getTotalLength() : event.getLength());
    }
}
